...
class CameraCalibration:
    ...
    def get_pose(self, object_points, image_points):
        return cv2.solvePnP(
            object_points, image_points, self.camera_matrix, self.distortion_coef
        )
...
class CalibrationApplication:
    ...
    def _display_calibration(self, camera_calibration):
        ...
        while True:
            ...
            image_points = camera_calibration.project_points(
                points, np.array([0.0, 0.0, 0.0]), np.array([0.0, 0.0, 0.0])
            )
            image_points = np.int0(image_points)
            cv2.line(
                image, tuple(image_points[0]), tuple(image_points[1]), (255, 255, 0), 3
            )
