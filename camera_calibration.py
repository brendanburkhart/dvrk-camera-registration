...
class CameraCalibration:
    ...
    def configure_image_size(self, shape):
        ...
        self.new_camera_matrix, self.roi = cv2.getOptimalNewCameraMatrix(
            self.camera_matrix, self.distortion_coef, (w, h), alpha, (w, h)
        )
    ...
    def get_pose(self, object_points, image_points):
        return cv2.solvePnP(
            object_points, image_points, self.new_camera_matrix, self.no_distortion
        )
...
class CalibrationApplication:
    ...
    def _compute_calibration(self, object_points, image_points, shape):
        ok, camera_matrix, distortion, rotation, translation = cv2.calibrateCamera(
            object_points, image_points, shape[::1], None, None
        )
        ...
        for i in range(len(object_points)):
            projected_points, _ = cv2.projectPoints(
                object_points[i], rotation[i], translation[i], camera_matrix, distortion
            )
            total_error += cv2.norm(
                image_points[i], projected_points, cv2.NORM_L2
            ) / len(object_points[i])
            total_error_2 += np.mean(
                np.linalg.norm(projected_points - image_points[i], axis=1)
            )

        mean_error = total_error / len(object_points)
        print(mean_error, total_error_2 / len(object_points))
    ...
    def _capture(self):
        ...
        while captures < self.min_captures:
            ...
            elapsed_time = (
                0 if last_capture_time is None else time.time() - last_capture_time
            )
            next_capture_ready = (
                last_capture_time is None or elapsed_time > self.capture_delay
            )
            ...
            if ret and next_capture_ready:
                ...
                captures += 1
                print(
                    "{} captures, need {} more".format(
                        captures, self.min_captures - captures
                    )
                )
    ...
    def run(self):
        ...
        ok, camera_matrix, distortion_coefs, error = self._compute_calibration(
        ...
        print(
            "Calibation saved to {} and {}".format(
                self.camera_matrix_file, self.distortion_coef_file
            )
        )
...
if __name__ == "__main__":
    ...
    print(
        'Print out a chessboard calibration pattern, e.g. "{:s}"'.format(
            chessboard_pattern_url
        )
    )
    print(
        "By convention, if your pattern has N rows and M columns, it is a (N-1)x(M-1) calibration pattern"
    )
    print(
        "Measure the size of the squares. If you are doing pose estimation, use the same units here"
    )
    print()
    print(
        "While program is running, slowly move pattern around on screen. If pattern is found, it will be"
    )
    print(
        "marked in a rainbow pattern and calibration captures will be taken. Try to cover variety of"
    )
    print(
        "pattern positions and angles, and make sure to get captures near edges of camera view."
    )
